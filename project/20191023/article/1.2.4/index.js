...
new Vue({
  ...
  watch:{
    ...
    'tagIndexes':{
      ...
      handler:function(val){
        ...
        _.keys(this.tagIndexes).forEach(function(v){
          arr.push({
            name:v,
            text:v,
            value:v
        ...
        $(this.$refs.tag).dropdown('change values',arr)
        console.dir(this.currentTag)
        $(this.$refs.tag).dropdown('set selected',this.currentTag)
      }
    },
    '$data':{
      deep:true,
  ...
  computed:{
    modeIcon:function(){
      return this.modeAction.list[this.modeAction.index].icon
    },
    emptyDraft:function(){
  ...
  methods:{
    changeMode:function(){
      this.modeAction.index = (this.modeAction.index + 1) % this.modeAction.list.length
      this.$router.push({
        path:this.modeIcon
      })
    },
    ...
    clickDocument:function(createdAt,indexes){
      switch(this.mode){
        case 'edit':
          indexes[createdAt] .editing = !indexes[createdAt] .editing 
  ...
  mounted:function(){
   ...
   ;
    ...
    localforage.getItem('walk', function(err,data) {
      if(data != null) {
        self.currentTag = data.currentTag
